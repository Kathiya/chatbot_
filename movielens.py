...
def ratings(src_filename='data/ratings.txt', delimiter='%', header=False, quoting=csv.QUOTE_MINIMAL):
	...
	reader = csv.reader(codecs.iterdecode(open(src_filename,"rb"), 'utf-8'), delimiter=delimiter, quoting=quoting)
	for line in reader:
...
def titles(src_filename='data/movies.txt', delimiter='%', header=False, quoting=csv.QUOTE_MINIMAL):
  ...
  reader = csv.reader(codecs.iterdecode(open('data/movies.txt',"rb"), 'utf-8'), delimiter=delimiter, quoting=quoting)
  title_list = []
  for line in reader:
